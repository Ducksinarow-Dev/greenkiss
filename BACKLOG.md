...
### Projects
- **#14: Project board columns.** Projects get a status board as the **default view**: **Upcoming** (assigned, not started), **In Progress** (currently working), **Approval** (ready for review), and **Done** hidden behind a slide-over panel (same treatment as Task Manager's Done column in #6). Board is one of two switchable views — a **List view** shows the same projects as vertical sections grouped by status instead of columns (view toggle persisted per user). Needs the Project status values remapped: current `active`/`on_hold`/`done`/`archived` → `upcoming`/`in_progress`/`approval`/`done` (+ keep `archived`), with a migration for existing projects and updated status pills everywhere projects render (list, detail header, My Dashboard cards).
