...
- **UI polish (#60/#61/#62):** ContentItemModal Channel is now a `<select>` (was wrapping pill buttons) sharing one row with a new `MultiSelectStaff` Assignees dropdown (keeps `assigneeIds[]`); `ProjectDetail` + `CampaignDetail` header cards trimmed to a compact one-line banner (padding/title/accent-bar/avatars), empty timeline hint hidden. (#32 plain-text search was already covered by #55.)
